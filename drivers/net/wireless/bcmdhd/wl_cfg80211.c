...
struct wl_netinfo_clean{
	struct work_struct wl_clean_work;
...
static void wl_cleanup_work(struct work_struct *work);
...
wl_cfg80211_del_virtual_iface(struct wiphy *wiphy, struct net_device *dev)
{
	...
	//bcm's patch for p2p panic s
	WL_DBG(("wait for interface deleted completely\n"));
	rtnl_unlock();
	msleep(1000);//harry
	rtnl_lock();
	//bcm's patch for p2p panic e
	return ret;
}
...
wl_cfg80211_ifdel_ops(struct net_device *ndev)
{
	...
	if (p2p_is_on(wl) && wl->p2p->vif_created &&
		...
		wl_cfgp2p_clear_management_ie(wl,
			index);
		//wl_clr_p2p_status(wl, IF_DELETING);//harry
		WL_DBG(("index : %d\n", index));
	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if (wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP) {
		...
		if(sta->idle >= 300){ //harry
             sta->idle = 150;
             WL_ERR((" STA Idle timeout, we skip here\n"));
                   }
...
static void wl_cleanup_work(struct work_struct *work){
	struct wl_netinfo_clean* wl_clean;
	wl_clean = container_of(work, struct wl_netinfo_clean, wl_clean_work);
	...
	void *ndev)
{
	...
	switch (state) {
		case NETDEV_UNREGISTER:
				...
				//add by cd_hwfu for wdev release issue
				INIT_WORK(&wl_netinfo_clean_s.wl_clean_work, wl_cleanup_work);
				wl_netinfo_clean_s.net=ndev;
				wl_netinfo_clean_s.wl=wl;
				queue_work(cfg80211_wq, &wl_netinfo_clean_s.wl_clean_work);
				break;
